...
void xiAPIplus_Enumerators::Init()
{
	...
	XI_SENSOR_FEATURE_SELECTOR_ITEMS.push_back({"XI_SENSOR_FEATURE_PARALLEL_ADC",         5, "When ON, all pixels are readout by 2-parallel ADC reducing the maximal framerate (to a half) and improving the image quality. This is available only on some cameras. (on/off)"});
	...
	XI_SENSOR_FEATURE_SELECTOR_ITEMS.push_back({"XI_SENSOR_FEATURE_HIGH_CONVERSION_GAIN", 9, "Serves as additional analog gain of 9.7 dB in pixels before conversion, essentially improving the image quality. This is available only on some cameras. (on/off)"});
